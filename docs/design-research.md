...
The selected direction is Workbench: a dark blue-black background, restrained typography, and an interactive project explorer. Each project presents its architecture stages, engineering decisions, evidence, limitations, and source links. The comparison gallery and other design directions have been removed at the owner's request.

The first screen identifies the engineer and reaches the selected work quickly. A recruiter can explore three projects; an engineer can follow decisions back to the source. Keyboard focus, vertical tab navigation, native links, mobile stacking, readable text, and reduced-motion support are built in.
...
The owner selected Workbench as the only portfolio. Its existing project explorer, architecture-stage controls, project illustrations, responsive layouts, and keyboard behavior are retained. Browser chrome and the initial HTML background match its blue-black (#0b1017) theme.

The original landing page, SQL demonstration, alternative designs, and comparison navigation are removed. Their implementation history remains in source control. The previous Workbench address redirects to the homepage.
